    ...
    public class Konsumen
    {
        ...
        public static Boolean CreateData(Konsumen k)
        {
            string sql = "insert into pelanggans (nama, email, password, telepon, saldo, poin) values ('" + 
                k.Nama.Replace("'", "\\'") + "','" + k.Email +"','" + k.Password +"','" + 
        ...
        public static List<Konsumen> ReadData(string kriteria, string nilaiKriteria)
        {
            ...
            while (hasil.Read() == true)
            {
                Konsumen k = new Konsumen(int.Parse(hasil.GetValue(0).ToString()), hasil.GetString(1),
                    hasil.GetString(2), hasil.GetString(3), hasil.GetString(4), float.Parse(hasil.GetString(5)),
                    int.Parse(hasil.GetString(6)));
                listKonsumen.Add(k);
            }
            return listKonsumen;
        }
